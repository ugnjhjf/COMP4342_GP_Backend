...
//            testchangeName();
    ...
    public static void testchangeName() throws Exception {
        System.out.println("Testing Change Name API...");

        // 构建 URL 对象，指定 API 的路径
        URL url = new URL(BASE_URL + "/changeName");
        HttpURLConnection connection = (HttpURLConnection) url.openConnection();
        ...
        // 创建 JSON 格式的请求体（包含 uid 和 newName）
        JSONObject jsonInput = new JSONObject();
        jsonInput.put("uid", 1);  // 假设 uid 为 1 的用户存在
        jsonInput.put("newName", "megumi");
